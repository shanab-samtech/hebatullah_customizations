...
class Analytics:
	...
	def get_sales_transactions_based_on_customers_or_suppliers(self):
		...
		# Add cost_center condition only if it's not empty
		if self.filters.cost_center:
			filters["cost_center"] = ["in", self.filters.cost_center]
	...
	def get_sales_transactions_based_on_items(self):
		...
		self.entries = (
			...
			.where(
				...
				& (doctype.company.isin(self.filters.company))
    			& (doctype.cost_center == self.filters.cost_center)
				& (doctype[self.date_field].between(self.filters.from_date, self.filters.to_date))
			)
		).run(as_dict=True)
	...
	def get_sales_transactions_based_on_customer_or_territory_group(self):
		...
		self.entries = frappe.get_all(
			...
			filters={
				"docstatus": 1,
				"company": ["in", self.filters.company],
    			"cost_center": ["in", self.filters.cost_center],
				self.date_field: ("between", [self.filters.from_date, self.filters.to_date]),
			},
